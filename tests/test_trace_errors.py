...
class TestTraceReaderErrors(unittest.TestCase):
    ...
    def test_open_retry_on_ebadf(self, mock_logger, mock_selector, mock_open):
        ...
        with patch('os.read') as mock_read:
            ...
            with patch.object(reader, '_reopen_if_needed', wraps=reader._reopen_if_needed) as mock_reopen:
    ...
    @patch('time.sleep')
    @patch('linux_edr.trace.logger')
    def test_file_unavailable_retry(self, mock_logger, mock_sleep, mock_selector, 
    ...
    @patch('selectors.DefaultSelector')
    @patch('linux_edr.trace.logger')
    def test_reopen_if_needed_with_exception(self, mock_logger, mock_selector, 
                                         mock_exists, mock_open):
